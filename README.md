...
This application is configured to work on Vercel with proper session management.

#### Required Environment Variables in Vercel

Set these in your Vercel project settings:

1. **SESSION_SECRET** (Required): A secure random string for session encryption
   ```bash
...
3. **VERCEL**: Automatically set by Vercel (don't override)

#### Optional Environment Variables

- **COOKIE_SAMESITE**: Cookie SameSite setting (`lax`, `strict`, or `none`)
  - Default: `lax` (recommended for Vercel)
  - Use `none` only if you need cross-domain cookies (requires `secure: true`)

- **COOKIE_DOMAIN**: Cookie domain (usually leave unset for Vercel)
  - Only set if using a custom domain with subdomains

- **ALLOWED_ORIGINS**: Comma-separated list of allowed origins for CORS
  - Default: Allow all origins
  - Example: `https://yourdomain.com,https://www.yourdomain.com`

- **DEBUG_SESSIONS**: Set to `true` to enable session debugging
  - Only use for troubleshooting
  - Access debug endpoint at `/api/debug/session`
...
- ✅ Rolling sessions (session expiration resets on activity)
- ✅ Proper SameSite cookie settings
- ✅ Session persistence across page navigation

#### Troubleshooting Vercel Sessions
...
2. **Check Cookie Settings**:
   - Visit `/api/debug/session` (if `DEBUG_SESSIONS=true`)
...
### Recommended Setup (Traditional Server)
